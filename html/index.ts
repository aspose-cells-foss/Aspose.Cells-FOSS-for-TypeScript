...
import { cellRef } from "../excel/util"
...
export class HtmlDocument {
  ...
  toWorkbook(options?: HtmlParseOptions): Workbook {
    ...
    for (const table of this._tables) {
      const worksheet = workbook.addWorksheet(table.caption || `Sheet${workbook.worksheetCount}`)
...
export class HtmlTable {
  ...
  toWorksheet(worksheet: Worksheet, options?: HtmlParseOptions) {
    const opts = { trimWhitespace: true, useFirstRowAsHeader: false, ...options }

    let startRow = 0

    if (opts.useFirstRowAsHeader && this._headers.length > 0) {
      for (let col = 0; col < this._headers.length; col++) {
        const value = opts.trimWhitespace ? this._headers[col].trim() : this._headers[col]
...
${tables.join("\n\n")}
  ...
  static fromWorksheet(worksheet: Worksheet, options?: HtmlSaveOptions): string {
    const table = new HtmlTable()
...
export function tableToHtml(table: HtmlTable, options?: HtmlSaveOptions): string {
  return table.toHtml(options)
}

export function workbookToHtml(workbook: Workbook, options?: HtmlSaveOptions): string {
  return HtmlWriter.fromWorkbook(workbook, options)
}

export function worksheetToHtml(worksheet: Worksheet, options?: HtmlSaveOptions): string {

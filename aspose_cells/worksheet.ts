...
export class Worksheet {
  ...
  toHtml(): string {
    ...
    for (let col = 0; col <= maxCol; col++) {
      const width = this._columnWidths.get(col) || 64;
      colWidths += `<col width='${width}' style='width:${width}pt'/>`;
    }

    let tableHtml = `<table border='0' cellpadding='0' cellspacing='0' width='${(maxCol + 1) * 64}' style='border-collapse:collapse;table-layout:fixed;width:${(maxCol + 1) * 64}pt'>
 ${colWidths}
`;

    for (let row = 0; row <= maxRow; row++) {
      tableHtml += ` <tr height='17' style='mso-height-source:userset;height:12.75pt'>`;
      ...
      for (let col = 0; col <= maxCol; col++) {
        ...
        if (isNumber) {
          tableHtml += `\n<td align='right'>${this.escapeHtml(value)}</td>`;
        } else {
